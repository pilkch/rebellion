...
bool cHeightmapData::LoadFromFile(const opengl::string_t& sFilename)
{
  voodoo::cImage image;
  if (!image.LoadFromFile(sFilename)) {
    std::cout<<"cHeightmapData::LoadFromFile Could not load "<<opengl::string::ToUTF8(sFilename)<<std::endl;
    return false;
  }
  ...
  for (size_t z = 0; z < size; ++z) {
    for (size_t x = 0; x < size; ++x) {
      ...
      while (
        ...
        LerpX = chris_round(CurrentPos.x);
        LerpZ = chris_round(CurrentPos.z);
